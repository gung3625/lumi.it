...
**⚠️ 미해결/다음**:
- 🔴 **전체 e2e 실측** — 위 개선들이 실제 충실도를 올렸는지 baseline 미검증(코드/단위테스트만 통과). 사장님 직접 e2e 진행 중이었음.
- 🔴 **create.html UX 구조변경** — 레퍼런스 미리보기(긴 이미지)가 폼을 밀어 '만들기' 버튼이 아래로·스크롤 필요 + 결과가 입력 아래 누적되어 페이지 계속 길어짐. 직전 작업 미완. 해결안: 미리보기 max-height 제한 + 생성시 입력폼 접기/결과 분리.
- 🟡 **과금 시스템** — 크레딧 블록 비례 차감 + 결제/충전 + 플랜(현재 무료 2회만, 유료 미구현). 마진모델=기본 500원+블록당 100원(마진 ~46%, 짧아도 흑자).
