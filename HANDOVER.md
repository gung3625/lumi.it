...
### ⭐⭐⭐ 2026-06-24 세션 — 편집/퀄리티 딜레마 해결방향 확정 [★현재 작업, 핸드폰 이어받기]

> **핵심 결론(반드시 먼저 읽을 것)**: gpt-image-2 통이미지(글자가 박힌 화보)는 화려하지만 **편집 불가**(글자가 그림이라서). 편집하려고 글자를 HTML로 빼면 **퀄리티가 죽음**(사장님 2번 거부). → 둘 다 동시엔 안 됨. **제디터/캔바 방식 = 템플릿(편집 가능한 레이어) + 블록편집**이 정답(퀄리티+편집 동시). 사장님 확정 "우리도 저렇게 가자".

**이번 세션 한 것**: 결과 영구저장(`r/{jobId}.html`→resultUrl), IP rate limit 5회/일(`generate-detail.js` allowRate), 도매토피아 파서(`dometopia.js`), **2단계 흐름**(분석 step→"이 정보 맞나요?" 확인·수정→생성, 거짓 차단 — `runAnalysis`/step='analyze'), 캡처 업로드, 사이트명·이모지 제거.

**편집 시도 2번 다 거부됨**: ①하이브리드(화보 글자X + HTML카피) ②오버레이(화보 위 흰글자+그라디언트) → 둘 다 "퀄리티 낮다/밋밋". → **롤백 완료**: `detail-page.js`를 `78e6c0e`(글자를 이미지에 박는 화려 버전)로 `git checkout` + 배포. **현 라이브 = 글자 박힌 화려 화보(편집 불가 상태).**

**확정 방향 (다음 작업 — 핸드폰서 이어받기)**:
- ① 통이미지 고집 버리고 → **레퍼런스 템플릿(퀄리티) + 편집 가능한 텍스트 레이어**(제디터식). 단계적으로(풀 블록에디터는 큰 작업).
- ② **레퍼런스 학습 기능**(사장님 아이디어 = hookable "레퍼런스 AI"): 고객이 마음에 드는 상세페이지를 붙여넣기 → AI가 색·무드·레이아웃·폰트 분석 → 그 스타일로 생성 → **저장 → 다음에 자동 재사용**. **사장님이 디자인 레퍼런스를 제공하기로 함**(내 디자인 감각이 약함이 입증됨 → 검증된 레퍼런스를 그대로 베끼는 게 확실).

**미정리(다음 세션 주의)**: ⚠️ `detail-maker.html`의 "직접 수정"(designMode) 버튼 = 이미지라 작동 안 함 → 제거/교체 필요. `generate-detail.js`의 `action==='save'` 분기 = 직접편집 폐기로 미사용(무해, 정리 가능). 참고영상 = 제디터 사용법 youtube `UJsFcJVtq2k`("피그마보다 10배 쉬움" = 블록 에디터 구조).
