...
- 워크트리 에이전트는 반드시 **워크트리 경로**의 파일만 수정 (메인 레포 절대 경로 사용 금지). 프롬프트에 워크트리 경로를 명시하지 않아도 에이전트는 자신의 작업 디렉토리(worktree) 내 파일만 편집해야 함
...
## 📸 캡션 플로우
- 사진 업로드 1회 → 캡션 1개 생성
- 마음에 안 들면 1회당 최대 3번 재생성 (현재 테스트용 999)
