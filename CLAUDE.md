...
- 에이전트는 반드시 `run_in_background: true`로 백그라운드 실행
...
## 📖 도메인 용어 (Phase 1 멀티마켓 SaaS)

- **베타 멀티마켓 SaaS**: 가격 미확정 (2026-04-29 사용자 명시 — 협의 중). 30명 한정, 가입비 0원, 카드 등록 X 정책만 유지
- **정식 출시 가격**: 미확정. 베타 데이터로 추후 결정
- **카피·UI 룰**: 어떤 정량 가격값(₩29,000·₩19,900 등)도 단정 표기 금지. "베타 신청 시 안내" 정도까지만
- 지원 마켓: 쿠팡 · 네이버 스마트스토어 · 토스쇼핑 (3사)
- 폐지된 Phase 0 가격(스탠다드 ₩19,900 / 프로 ₩29,900 / 비즈니스): 셀러향 카피·UI에서 노출 금지
- 인스타 캡션·DM AI 자동응답: Phase 1 비활성 (Meta 심사 통과 + 별도 동의 후 활성), 셀러 카피에 어필 X
- 사업자등록번호: **404-09-66416** (모든 페이지 통일)
- 서비스 상태: Phase 1 베타, Meta 심사 중, 멀티마켓 SaaS 30명 한정 모집
