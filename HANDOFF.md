...
마지막 업데이트: 2026-05-14
기준 커밋: `main` 최신 (`ae67504`) — PR #69~#240 170+ 건 + 2026-05-14 직접 main 커밋 5건 (메타 비즈니스 인증 대비 + 탭바 padding 일부 복구)

---

## 🚨 미해결 — 다음 세션 우선 진단 (2026-05-14)

### 하단 탭바 `position: fixed` 가 사장님 iPhone Safari 에서 깨짐

**증상:** 사장님이 끝까지 스크롤하면 탭바가 viewport 하단에 고정 안 되고
*footer 아래 자연 위치*. 즉 `position: fixed` 가 `static` 처럼 동작.

**시도한 fix (모두 실패 — 사장님 디바이스 검증 X):**
1. PR #233 — `body padding-bottom 56→80px` (탭바 높이 보정)
2. PR #238 — `body { overflow-x: hidden }` → `overflow-x: clip`
   - 가설: iOS Safari 가 overflow-x:hidden 시 fixed 의 containing block 을
     viewport → body 로 만드는 알려진 버그
3. PR #239 — body 의 overflow-x 자체 제거
4. PR #240 — body padding-bottom 의 80px reservation 제거 (사장님 통찰
   "footer 아래 빈 영역 자체 차단"). 단 *fixed 가 작동 안 하는 근본 원인은
   여전히 미해결* — padding 제거로 사용자 시각만 가림.

**다음 세션 진단 단서:**
- 사장님 디바이스: iPhone, iOS Safari 18, 주소창 상단 모드
- 라이브 CSS 확인 결과 `position: fixed` 정상 적용, override 없음, body
  overflow 도 제거됨. 그럼에도 fixed 깨짐.
- 가능한 다른 원인 (미검증):
  - 부모 element 의 `transform`/`filter`/`contain`/`will-change` —
    `containing block` 생성으로 fixed 기준 변동
  - `<html>` 자체에 overflow 설정 (확인 안 됨)
  - PWA Standalone mode (홈 화면 추가) 에서 fixed 동작 차이
  - `iOS Safari + 100vh / 100dvh` viewport 단위 버그
  - meta viewport 의 `viewport-fit=cover` 가 영향?
  - iOS Safari Web Inspector 로 직접 inspect 필요
- 사장님이 *진단 디버그* 위해 다른 세션 진행 의향

**추가 변경 — 탭바 미해결 후속으로 사장님 시각 임시 가림:**
- PR #240 의 footer padding 제거로 *fixed 가 깨진 환경에서도* 빈 영역
  안 보임. 단 *fixed 가 정상인 환경* 에선 footer 마지막 줄이 탭바에
  부분 가림 — trade-off 받아들임 (footer = 사업자 정보, 자주 보는 영역 X)

**2026-05-14 갱신 — 메타 비즈니스 인증 대비로 footer 가림 재해결 시도 (commit `315aeac`/`7196866`/`ae67504`):**
- 사장님 환경에서 푸터(사업자 정보 카드 추가됨)가 탭바에 가려진다고 다시 보고.
- `css/tabbar.css` 에 `body:has(.tabbar) { padding-bottom: calc(80px + env(safe-area-inset-bottom)); }` 추가 (PR #240 에서 제거한 reservation 80px 재도입). `:has()` 셀렉터로 탭바 있는 7개 페이지에만 자동 적용.
- 사장님 통찰 "다른 페이지는 OK 인데 트렌드만 푸터 가린다" → trends 만 inline `<style>` 에서 110px override (다른 페이지 80px 유지).
- **trade-off 재정의:** fixed 가 정상인 환경(iOS Safari 정상·데스크탑·Android Chrome)에선 푸터 정상 노출. **fixed 가 깨진 iPhone Safari 환경에서는 다시 빈 영역 발생 가능** — 사장님 환경 변화(iOS 업데이트?) 가정.
- **다음 세션 의무 점검:** 사장님 iPhone Safari 에서 빈 영역이 다시 보이는지 확인. 보이면 fixed 깨짐 근본 진단(부모 transform/contain/will-change 등) 으로 곧장 진입.
...
### 2026-05-14 (오후) — 메타 비즈니스 인증 대비 사이트 보강 + 탭바 padding 일부 복구
...
**미해결 / 후속:**
- 사장님 iPhone Safari 의 `position: fixed` 깨짐 (PR #233~#240 시리즈) 은 여전히 미해결. §🚨 미해결 참조.
...
#### 하단 탭바 fixed 깨짐 (#233, #238, #239, #240) — 미해결
...
- **상태: 미해결.** 위 §🚨 미해결 섹션 참고. 다음 세션이 iOS Safari Web
  Inspector / DOM tree 부모 transform 등 직접 진단 필요.
