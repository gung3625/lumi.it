...
마지막 업데이트: 2026-05-12
기준 커밋: `main` 최신 (`060b4cd`, PR #147 머지) — PR #69~#147 78건 반영
...
### PR #171 ~ #178 — 트렌드 v2 신호 강화 + 실 검색량 ranking (2026-05-12)

사용자 피드백 흐름 ("순위 근거 뭐야 / 실 검색량 확인해야지 / 의미 없는 소스 빼") 따라
트렌드 인프라 본격 강화. 8 PR.

#### 신호 소스
- **#171 네이버 쇼핑인사이트 통합** — `_shared/naver-shopping-insight.js` 복원
  (옛 멀티마켓 SaaS 시절 코드, 2026-04 작성 → 5월 피보팅 정리됐던 것). 8 lumi
  업종 → 네이버 카테고리 매핑 (식품/화장품/패션의류+잡화/가구·인테리어/스포츠).
  `fetchNaverShoppingSignals` 가 trends-v2 cron 에서 카테고리당 trend / age /
  gender 3 endpoint 호출. demographics 를 GPT 분류 prompt 에 컨텍스트 주입
  ("30대 여성 73%" → 캡션 톤 결정에 활용). `SOURCE_WEIGHTS.shopping=3`.

- **#177 Community 소스 전수 제거** — 맘카페/디시/더쿠 GPT 웹 검색 cron
  (`scheduled-community-trends-background`) 폐기. 데이터 양 0건 (cron 작동 안 함)
  이라 의미 미미. 파일 삭제 + netlify.toml redirect 정리 + DB cache row 정리.

- **#178 Google Trends 소스 전수 제거** — 글로벌 일일 핫토픽 (정치/이슈) 이라
  lumi 9 카테고리 매칭률 거의 0. 40 entries / 9 카테고리 = ~4건/cat, 무의미.
  `google-trends-api` npm 의존성도 제거.

#### Ranking 강화 (실 검색량 기반)
- **#172 Top 10 한정 + "왜 이 순위?" sheet 노출** — trends.html slice(0, 30) → 10.
  sheet 상세에 velocity / cross-source count / weighted_score 근거 라벨링.
  정렬 식 사장님에게 노출.

- **#173 네이버 검색광고 API 월간 검색량 (Layer 1)** — 기존 `_shared/naver-ad-keyword-tool.js`
  (시드 확장용으로만 쓰이던 helper) 를 ranking 기반으로 확장. `fetchKeywordSearchVolume`
  추가 — 단일 키워드의 월간 PC+모바일 합산. `trend_keywords.monthly_search_total`
  컬럼 + ranking 1차 정렬 기준. monthlyTotal 자체가 1달 누적이라 1일 spike
  자동 감쇄 (사용자 우려 "그 순간 신호로 1위" 해결).

- **#175 Multi-layer fallback (Option F max quality)** — Layer 2 + 3 추가.
  - Layer 2: 한국어 합성어 분해 휴리스틱 (`splitCompoundKorean`) → root 키워드
    매칭 (예: "수국드라이플라워" → "드라이플라워" / "글로우파운데이션" → "글로우")
  - Layer 3: DataLab ratio cross-reference 환산 — 같은 카테고리 exact-matched
    키워드를 anchor 로 잡고 DataLab `/search` 로 [anchor, target] ratio 받기.
    estimate = anchor.monthlyTotal × (targetRatio/anchorRatio).
  - 신뢰도 가중 ranking: exact 1.0 / normalized 0.95 / root_morpheme 0.6 /
    datalab_estimate 0.4 / null → weightedScore fallback
  - `trend_keywords.search_volume_match_type` + `search_volume_root_keyword`
    DB 컬럼 추가. 사장님에게 정직 라벨링 (정확/근사/추정/미수집).
  - UI: kw-badge--vol (파랑, 정확) / kw-badge--vol-approx (골든, 근사)

- **#176 monthlyTotal === 0 매칭 거부** — Layer 2 휴리스틱이 의미 없는 substring
  ("자라버뮤", "치네일") 으로 검색 시 Naver API 가 0 row 반환 → 옛 코드는
  valid hit 으로 적재. fetchKeywordSearchVolume 에서 `monthlyTotal <= 0` 거부 →
  다음 candidate 또는 Layer 3 fallback.

#### 적재 결과 (2026-05-12 cron, 첫 검증)
- exact: 3~5건 / 카테고리
- root_morpheme: 4~10건 / 카테고리
- datalab_estimate: 4~11건 / 카테고리
- null (모든 layer 실패): 절반 정도 — 진짜 니치 키워드 + Naver DB 미수록

#### 잔재 위생
- **#170 HANDOFF.md + admin-demo-test-sync.js 삭제** — 세션 중간 정리.
- **#174 debug probe** — search-volume null 추적용 임시. admin-naver-ad-probe.js
  에 volumeResults 출력 추가. **추후 정리 권장** (PR #174 의 디버그 코드는
  production 영향 0 이라 시급 X).

#### 다음 단계 (미진행, 후속 작업)

**Threads keyword_search API 통합** — IG 보다 우월한 트렌드 수집 옵션 발견.
조사 결과:
- Endpoint: `https://graph.threads.net/v1.0/{user-id}/keyword_search?search_mode=TAG`
- Rate limit: 24h 2,200 쿼리 (lumi 사용량 대비 충분)
- 검색 범위: 모든 공개 게시물 (IG hashtag-business-discovery 보다 광범위)
- Z세대 활성도 ↑
- **lumi 가 이미 관리자 단일 Threads 토큰 보유** (env: `THREADS_ACCESS_TOKEN`,
  `THREADS_USER_ID` — select-and-post-background.js 의 postToThreads 가 사용).
  그 토큰을 keyword_search 에 재사용 → 사용자 OAuth 흐름 신규 작업 불필요.
- 작업량 ~3h. 본인이 Meta 개발자 대시보드에서 `threads_keyword_search` 권한
  추가 → 토큰 재발급 → cron 신규 작성.

**IG hashtag 캐시 복구** — 코드 (`scheduled-ig-hashtag-background.js`) 정상.
PR #165 의 DB 클린 슬레이트 후 `ig_accounts` 가 0 이라 `LUMI_BRAND_USER_ID` env
가 가리키는 사용자의 토큰 조회 실패. 본인이 카카오 가입 + IG OAuth 연동 +
LUMI_BRAND_USER_ID env 등록 후 자동 작동.

**TikTok Creative Center 통합** — Z세대 트렌드 신호 추가. OAuth 무관 (Creative
Center 페이지 스크래핑). 작업량 ~4h.
