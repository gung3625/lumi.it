...
2. **베스트 시간 응답 매핑** — 클라이언트 `loadBestTime` 이 `bestTime` 필드 사용. 동작 OK. 필요 시 응답 형식 보강.
3. **댓글 Phase 2** — `comments.js` 가 IG Graph API 로 실제 댓글 fetch (현재는 빈 배열)

### 🟡 보강
- **Storage orphan 정리** — 매시간 cleanup-stale 은 reservations row 기준이라 row 없는 storage 파일은 누적. 별도 cron 또는 prefix 스캔 필요.
- **사진 압축 quality 동적 조정** — 9-10장 업로드 시 6MB 한도 임박. quality 0.85 fallback 로직 추가 가능.
