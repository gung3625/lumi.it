#!/usr/bin/env node
// Sprint 2 — netlify dev 대체 미니 서버
// Sprint 1 mini-server 패턴 그대로 + Sprint 2 라우트 4종 + register-product.html
...
// Sprint 2 기본 모킹 — JWT 직접 발급 + Supabase 미사용
...
const MULTIPART_ROUTES = new Set(['/api/upload-business-license', '/api/upload-product-image']);
...
server.listen(PORT, () => {
  console.log(`Sprint 2 mini server: http://localhost:${PORT}`);
  console.log(`MOCK: SIGNUP=${process.env.SIGNUP_MOCK} AI=${process.env.AI_PRODUCT_ANALYZE_MOCK} COUPANG=${process.env.COUPANG_VERIFY_MOCK} NAVER=${process.env.NAVER_VERIFY_MOCK}`);
});
