...
--   marketplace_orders + inventory_movements + cs_threads + cs_messages + tracking_events + kill_switch_log
-- 적용 방법: Supabase SQL Editor에서 직접 실행
-- 멱등(idempotent)하게 작성 — 중복 실행 안전
--
-- ⚠️ 테이블명 주의: `marketplace_orders` 사용 (기존 PortOne 결제용 `orders`와 충돌 회피)
-- =========================================================================

-- =========================================================================
-- 1. marketplace_orders — 마켓 주문 본문 (마켓 → 루미 수집 + 루미 → 마켓 송장 + 역방향 반품)
-- =========================================================================
...
CREATE INDEX IF NOT EXISTS idx_marketplace_orders_seller_status ON marketplace_orders(seller_id, status, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_marketplace_orders_tracking ON marketplace_orders(tracking_number) WHERE tracking_number IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_marketplace_orders_market ON marketplace_orders(market, market_order_id);
CREATE INDEX IF NOT EXISTS idx_marketplace_orders_returned ON marketplace_orders(seller_id, return_requested_at DESC) WHERE return_requested_at IS NOT NULL;
...
DROP POLICY IF EXISTS "Sellers manage own orders" ON marketplace_orders;
CREATE POLICY "Sellers manage own orders" ON marketplace_orders
  USING (seller_id::text = (current_setting('request.jwt.claims', true)::jsonb ->> 'seller_id'));

COMMENT ON TABLE marketplace_orders IS 'Sprint 3 마켓 주문 (정방향 + 역방향). buyer_*_masked 컬럼만 — 평문 개인정보 절대 금지. 기존 PortOne 결제용 orders 테이블과 별도.';
COMMENT ON COLUMN marketplace_orders.status IS 'received(접수)→paid(결제완료)→shipping(배송중)→delivered(배송완료) / returned/exchanged/cancelled(역방향)';
...
-- 마이그레이션 완료 검증 쿼리
-- =========================================================================
-- SELECT count(*) FROM information_schema.tables WHERE table_schema='public' AND table_name IN ('marketplace_orders','inventory_movements','cs_threads','cs_messages','tracking_events','kill_switch_log','courier_codes');
-- → 7
