...
const FILLER_WORDS = [
  ...
  '관계자', '업계', '전문가', '시민', '네티즌',
  // 카테고리 오분류 방지: '인테리어'는 가구/소품 분야의 카테고리 총칭이라
  // 어떤 업종(cafe/food/beauty/...)의 트렌드 키워드로도 무의미.
  // "우드인테리어/한옥인테리어/카페인테리어" 등 부분 일치로 일괄 제거.
  '인테리어',
];
...
async function mergeV2Fields(supa, keywords, category, collectedDate, axisFilter, region = 'all', subcatFilter = null) {
  try {
    ...
    // axis 화이트리스트 적용. signal_tier 무관 (weak 도 정렬 후순위로 포함).
