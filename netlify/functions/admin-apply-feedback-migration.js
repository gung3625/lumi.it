// admin-apply-feedback-migration.js — user_trend_feedback 테이블 마이그레이션 실행기
// POST /api/admin-apply-feedback-migration
...
exports.handler = async (event) => {
  ...
  try {
    // migrations/003-user-trend-feedback.sql 읽기
    const sqlPath = path.join(__dirname, '..', '..', 'migrations', '003-user-trend-feedback.sql');
    let sql;
    try {
      sql = fs.readFileSync(sqlPath, 'utf8');
    } catch (readErr) {
      console.error('[admin-apply-feedback-migration] SQL 파일 읽기 실패:', readErr.message);
    ...
    for (const [label, connStr] of attempts) {
      ...
      try {
        ...
        applied = true;
        await client.end();
        console.log(`[admin-apply-feedback-migration] connected via ${label}`);
        break;
      } catch (e) {
    ...
    if (!applied) {
      ...
      console.error(`[admin-apply-feedback-migration] DB 연결 실패: ${dbg} | ${errors.join(' | ')}`);
      return { statusCode: 500, headers, body: JSON.stringify({ error: '마이그레이션 적용 실패' }) };
    }

    console.log('[admin-apply-feedback-migration] 003-user-trend-feedback.sql 적용 완료');
    return {
      statusCode: 200,
      headers,
      body: JSON.stringify({ success: true, file: '003-user-trend-feedback.sql' }),
    };
  } catch (err) {
    console.error('[admin-apply-feedback-migration] 예외:', err.message);
    return { statusCode: 500, headers, body: JSON.stringify({ error: '마이그레이션 적용 실패' }) };
  }
};
