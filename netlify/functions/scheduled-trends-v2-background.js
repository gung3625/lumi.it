...
const DEFAULT_TRENDS = {
  ...
  food: ['오마카세', '파스타', '한식주점', '수제버거', '스몰디쉬', '구이전문점', '와인바'],
...
const NAVER_KEYWORDS = {
  ...
  food: [
    { groupName: '브랜드맛집', keywords: ['다운타우너버거'] },
...
async function estimateByDataLabRatio(anchor, target) {
  ...
  // 100 미만 추정은 noise — reject (검증 2026-05-13).
  if (monthlyTotal < 100) return null;
  return {
    monthlyTotal,
...
**각 키워드는 가장 적합한 단일 카테고리에만 배치하세요.** 한 키워드를 여러 카테고리에 중복 배치하지 마세요.

자주 헷갈리는 경계 — 다음 룰 절대 준수:
...
- "참나무장작구이"·"야장감성"·"오마카세"·"이자카야"·"와인바"·"주점"·"고기집" → **food 전용 (cafe 배치 절대 금지)**
- "라떼"·"크로플"·"소금빵"·"마들렌"·"디저트"·"베이커리"·"케이크"·"빵"·"마카롱"·"스콘"·"푸딩"·"빙수"·"플라워케이크"·"꽃모양케이크" → **cafe 전용 (food·flower 배치 절대 금지)** — 디저트·베이커리류는 모두 cafe. 꽃 모양 장식 디저트도 실제 꽃이 아니므로 flower 금지
- "강아지간식"·"고양이사료"·"반려견수제간식"·"강아지수제간식"·"펫쿠키"·"펫푸드" → **pet 전용 (food 배치 절대 금지)** — 반려동물용 사료·간식은 사람 음식이 아님

**중요**:
- 고기·술·주점 감성은 무조건 food. 카페는 음료·디저트·빵·커피 공간.
- 반려동물용 사료·간식은 모두 pet. 사람 food 아님.
- 애매한 키워드는 가장 핵심 의미 한 곳에만. 정 모르면 빈 배열이 정답.
...
## 절대 준수 — 대중적 인지도 필수 (사용자 최우선 지시 2026-05-13)
**한국에서 월 검색량 1000회 이상으로 추정될 만한 키워드만 선별**.
실제로 사람들이 검색하지 않는 마이너 브랜드+제품 합성어는 **트렌드가 아니라
허구 키워드** — 절대 출력 금지. lumi 가 키워드별 실 검색량을 사후 검증하는데,
검색량 10~30회짜리 niche 합성어가 TOP 10 에 노출되는 사고 반복.

❌ 절대 금지 — "마이너 브랜드 + 제품" 합성어 (실 검색량 0~100):
  - 에코피니티세럼, 닥터지선플러스, 올가휴선앰플, 트리폴라토너패드, 율무크림,
    테라로직선크림, 닥터지브라이트닝업, 레디우스판테알엔, 레디우스크림
  - 자라레이스탑, 자라데님스커트, 자라스트라이프니트, 자라미니백,
    자라버뮤다팬츠, 자라니트재킷, 자라롱항공점퍼, 자라슬리브리스원피스,
    자라여름샌들, 자라와이드팬츠
  - 강아지유산균브랜드, 딩고간식, 츄잇육포, 블루벨하네스
  - 단발레이어드컷, 숏볼륨펌, 히피펌, 내추럴리즘 (스타일 합성어 중 실 검색량 적은 것)
  → 위 패턴은 GPT 가 만든 *허구 합성어*. 한국에서 실제로 거의 검색되지 않음.

✅ 허용 — 대중 인지도 + 검색 흔적 명확한 키워드:
  - 메가 인기 메뉴/제품: 마라탕, 버터떡, 우베라떼, 말차라떼, 티라미수, 크로플,
    아메리카노, 카페라떼, 짜장면, 김밥
  - 메가 인기 스타일: 허쉬컷, 시스루뱅, 오로라네일, 젤네일, 발레아쥬
  - 대중 인지도 브랜드 (제품명 X, 브랜드 단독): 자라, 유니클로, 룰루레몬, 호카,
    나이키, 아디다스, 닥터지, 코스알엑스, 라네즈, 이니스프리, 로얄캐닌, 아카나
  - 대표 메뉴 단독: 오마카세, 수제버거, 와인바, 이자카야, 한식주점

판정 기준 (스스로 묻기):
  "내가 친구한테 이 키워드 검색해보라고 하면, 친구가 무엇인지 *아는가*?"
  - 아는 거 → 출력 OK
  - "그게 뭔데?" → 너무 niche, 출력 금지

수량 채우려고 마이너 합성어 만들지 말 것. **빈 배열이 정답**.
...
4. **"X + 업종/형태" 합성어** (사용자 최우선 지시 2026-05-14): 한식주점, 일식주점, 한식당, 중식당, 분식집, 디저트카페, 브런치카페, 한옥카페, 감성카페, 한식맛집, 일식맛집, 헤어샵, 네일샵, 미용실, 피부과, 와인바, 맥주바
   → 사람들이 검색하는 건 **구체 메뉴·상품·스타일** (마라탕·오마카세·흑임자크림라떼) 이지 업종 자체가 아님. 한식주점은 "한식" + "주점" 두 형태 단어의 단순 결합 — 트렌드 X.
