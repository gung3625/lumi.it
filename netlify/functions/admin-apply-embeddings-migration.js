// admin-apply-embeddings-migration.js — Phase 4 임베딩 마이그레이션 실행기
// POST /api/admin-apply-embeddings-migration
...
exports.handler = async (event) => {
  ...
  try {
    const sqlPath = path.join(__dirname, '..', '..', 'migrations', '006-trend-embeddings.sql');
    let sql;
    try {
      sql = fs.readFileSync(sqlPath, 'utf8');
    } catch (readErr) {
      console.error('[admin-apply-embeddings-migration] SQL 파일 읽기 실패:', readErr.message);
    ...
    for (const [label, connStr] of attempts) {
      ...
      try {
        ...
        applied = true;
        await client.end();
        console.log(`[admin-apply-embeddings-migration] connected via ${label}`);
        break;
      } catch (e) {
    ...
    if (!applied) {
      ...
      console.error(`[admin-apply-embeddings-migration] DB 연결 실패: ${dbg} | ${errors.join(' | ')}`);
      return { statusCode: 500, headers, body: JSON.stringify({ error: '마이그레이션 적용 실패' }) };
    }

    console.log('[admin-apply-embeddings-migration] 006-trend-embeddings.sql 적용 완료');
    return {
      statusCode: 200,
      headers,
      body: JSON.stringify({ success: true, file: '006-trend-embeddings.sql' }),
    };
  } catch (err) {
    console.error('[admin-apply-embeddings-migration] 예외:', err.message);
    return { statusCode: 500, headers, body: JSON.stringify({ error: '마이그레이션 적용 실패' }) };
  }
};
