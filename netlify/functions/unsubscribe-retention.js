  ...
  try {
    ...
    if (!email || !token) {
      return {
        statusCode: 400,
        headers,
        body: buildPage('잘못된 요청', '유효하지 않은 수신거부 링크입니다.')
      };
    }
    ...
    if (token !== expectedToken) {
      return {
        statusCode: 403,
        headers,
        body: buildPage('인증 실패', '유효하지 않은 수신거부 링크입니다.')
      };
    }
    ...
    if (!raw) {
      return {
        statusCode: 404,
        headers,
        body: buildPage('사용자 없음', '해당 이메일로 등록된 사용자를 찾을 수 없습니다.')
      };
    }

    const user = JSON.parse(raw);
    user.retentionUnsubscribed = true;
    user.retentionUnsubscribedAt = new Date().toISOString();
    await store.set('user:' + email, JSON.stringify(user));

    return {
      statusCode: 200,
      headers,
      body: buildPage('수신 해제 완료', '리텐션 이메일 수신이 해제되었습니다.<br>더 이상 마케팅 이메일을 받지 않으실 거예요.')
    };
  } catch (err) {
    ...
    return {
      statusCode: 500,
      headers,
      body: buildPage('오류 발생', '처리 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요.')
    };
  }
};
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .logo { font-size: 28px; font-weight: 800; color: #FF6B9D; margin-bottom: 24px; letter-spacing: -0.5px; }
