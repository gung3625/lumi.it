...
exports.handler = async (event) => {
  ...
  try {
    ...
    return { statusCode: 400, headers: headers, body: JSON.stringify({ error: 'JSON 본문 파싱 실패' }) };
  }

  const contentType = (body.contentType || '').toLowerCase();
  if (!ALLOWED_MIME.has(contentType)) {
    return { statusCode: 400, headers: headers, body: JSON.stringify({ error: '이미지 형식(JPG/PNG/WebP/GIF)만 업로드 가능해요.' }) };
  }
  const base64 = typeof body.base64 === 'string' ? body.base64.replace(/^data:[^;]+;base64,/, '') : '';
  if (!base64) {
    return { statusCode: 400, headers: headers, body: JSON.stringify({ error: 'base64 본문이 비어있어요.' }) };
  }

  const buffer = Buffer.from(base64, 'base64');
  if (!buffer.length) {
    return { statusCode: 400, headers: headers, body: JSON.stringify({ error: 'base64 디코딩 실패' }) };
  }
  if (buffer.length > MAX_BYTES) {
    return { statusCode: 413, headers: headers, body: JSON.stringify({ error: '파일이 너무 커요 (최대 5MB).' }) };
  }
  ...
  try {
    ...
    if (upErr) {
      ...
      return { statusCode: 500, headers: headers, body: JSON.stringify({ error: '업로드 실패' }) };
    }
