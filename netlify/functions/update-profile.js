...
exports.handler = async (event) => {
  ...
  if (Object.keys(update).length === 0) {
    return { statusCode: 400, headers: headers, body: JSON.stringify({ error: '변경할 내용이 없습니다.' }) };
  }
  ...
  try {
    ...
    if (error) {
      ...
      return { statusCode: 500, headers: headers, body: JSON.stringify({ error: '저장 중 오류가 발생했습니다.' }) };
    }

    return { statusCode: 200, headers: headers, body: JSON.stringify({ success: true, user: data }) };
  } catch (err) {
