...
- 사진에 보이는 것만 분석. 계절/날씨/트렌드는 별도 제공되니 추측하지 마세요.
...
**[첫 문장 후보]** 스크롤을 멈추게 만드는 캡션 첫 문장 3개.
- 하나는 질문형 ("이 색감 실화야?")
- 하나는 감성형 ("창밖 빗소리랑 라떼 한 잔이면 충분한 오후.")
- 하나는 직관형 ("새로 나온 딸기 라떼, 비주얼부터 다름.")`;
...
### 법적 위험 표현
- "무첨가", "100% 천연", "유기농" — 인증 없이 사용 금지 (식품위생법 위반)
- "의사 추천", "효과 보장", "치료" — 의료법 위반 (뷰티/헬스)
- "특허", "인증", "수상" — 실제 인증 여부 모르면 사용 금지 (허위광고)
- 타인 초상권/저작권 관련 표현 금지

### 감정/톤 위험
- 고객을 깎아내리는 표현 금지 ("아직도 모르세요?", "이것도 몰라요?")
- 강요하는 톤 금지 ("꼭 와야 해요", "안 오면 후회")
- 불안 조성 금지 ("품절 임박", "마지막 기회", "놓치면 끝")
- 비교/비하 금지 ("다른 데랑 비교 불가", "여기만 가능한")

### 개인정보/프라이버시
- 고객 이름, 전화번호 등 개인정보 노출 금지
- "단골 OO님" 같은 특정 고객 지칭 금지
- 매장 직원 실명 언급 금지 (동의 없이)

### 성적/선정적 표현
- 성적 암시 표현 금지
- 외모 평가 금지 ("예쁜 사장님이 만든")
- 신체 관련 부적절한 표현 금지 (뷰티/헬스 업종 특히 주의)

### 기타
- 욕설, 비속어, 은어 금지
- 다른 매장/경쟁사 비방 금지
- 저작권 있는 노래 가사, 영화 대사 인용 금지
- 특정 연예인/유명인 이름을 동의 없이 사용 금지
- 사진에 없는 메뉴를 트렌드라고 넣지 말 것 (예: 라떼 사진인데 "크로플도 준비 중")
- "이번 주까지만", "4월 한정" 같은 시간/시기 단정 금지 (매장 일정 모름)
- "손님들이 다 좋아하세요", "리뷰 폭발" 같은 고객 반응 날조 금지

## 이런 캡션을 쓰세요
- 이미지 분석의 [첫인상]을 캡션 첫 문장의 감성 씨앗으로 활용하세요
- 이미지 분석의 [첫 문장 후보]를 참고하되 그대로 베끼지 마세요
- 첫 문장에서 스크롤이 멈춤
- 다음 문장이 궁금해서 계속 읽게 됨
- 대표님이 "이게 내가 하고 싶었던 말이야" 라고 느낌
- 이모지는 2~4개, 문장의 감정을 보완하는 위치에만 (연속 사용 금지)
- 마지막 문장은 팔로워의 행동을 유도:
...
${item.userMessage ? '\n⚠️ 대표님 코멘트가 있으면 이것이 캡션의 핵심 메시지입니다. 코멘트 내용을 반드시 캡션에 우선 반영하세요. 사진 분석과 트렌드는 코멘트를 보조하는 역할입니다.' : ''}
...
해시태그 총 8~12개를 아래 비율로 구성:
- 대형 (검색량 100만+): 1~2개 (예: #카페스타그램, #맛집추천)
- 중형 (1만~10만): 3~4개 (예: #성수카페, #봄디저트)  
- 소형 (1천~1만): 2~3개 (예: #성수동카페추천, #딸기라떼맛집)
- 트렌드: 트렌드 태그에서 **사진 내용과 직접 관련 있는 것만** 2~3개 포함
- 지역 태그: 매장 지역이 있으면 1개 (예: #마포맛집)
...
## 캡션 3개 (반드시 3개, 각각 확실히 다르게)

**버전 1 — 스토리텔링** (5~8줄)
이미지에서 느껴지는 감성을 스토리로 풀어가는 캡션. 읽는 사람이 그 공간에 있는 듯한 몰입감.

**버전 2 — 짧고 강렬** (2~3줄)
핵심만. 한두 문장으로 임팩트. 여백이 힘. 바쁜 사장님이 바로 쓸 수 있는 캡션.

**버전 3 — 정보형** (4~6줄)
메뉴/제품/서비스 정보를 자연스럽게 녹인 캡션. 설명하는 느낌 없이 "알려주는 척 이야기하는" 톤.
