...
async function synthesizeWithGPT(category, naverData, googleData) {
  ...
  const prompt = `당신은 한국 인스타그램 트렌드 분석가입니다.

오늘 날짜 기준, "${categoryKo}" 업종에서 실제로 유행하고 있는 구체적인 트렌드 키워드 8개를 선정하세요.
...
선정 기준 (반드시 지킬 것):
- "지금 유행하는 구체적인 것" (예: #큐빅네일, #말차라떼, #크로플)
- 특정 스타일, 메뉴, 기법, 시즌 아이템 등 구체적 대상
- 검색량이 최근 급상승한 키워드 우선

제외 기준 (절대 포함하지 말 것):
- 뻔한 카테고리 단어: 카페, 커피, 네일, 뷰티, 맛집, 디저트
- 콘텐츠 주제형: ~아이디어, ~추천, ~방법, ~정보, ~모음, ~팁
- 상시 검색어: 인스타그램, 스타그램, 좋아요, 팔로우
- 경쟁 브랜드명: 스타벅스, 이디야, 투썸 등
- 영어 일반 단어: coffee, dessert, menu, food

# 포함, JSON 배열만 응답.
예시: ["#큐빅네일","#말차디저트","#크로플","#젤리네일","#오마카세","#봄신메뉴","#딸기시즌","#핸드드립"]`;
