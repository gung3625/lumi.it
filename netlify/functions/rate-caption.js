...
exports.handler = async (event) => {
  ...
  if (event.httpMethod !== 'POST') {
    return { statusCode: 405, headers: CORS, body: JSON.stringify({ error: 'POST 전용' }) };
  }
  ...
  try {
    ...
    return { statusCode: 400, headers: CORS, body: JSON.stringify({ error: '잘못된 JSON' }) };
  }

  const { reservation_id, rating } = body;
  if (!reservation_id || typeof reservation_id !== 'number') {
    return { statusCode: 400, headers: CORS, body: JSON.stringify({ error: 'reservation_id 필수 (number)' }) };
  }
  if (!['like', 'dislike', 'skip'].includes(rating)) {
    return { statusCode: 400, headers: CORS, body: JSON.stringify({ error: 'rating은 like | dislike | skip 중 하나' }) };
  }
  ...
  try {
    ...
    if (reservation.user_id !== user.id) {
      return { statusCode: 403, headers: CORS, body: JSON.stringify({ error: '접근 권한 없음' }) };
    }
